...
function parseFrontMatter(text,file=""){
 ...
 return {...meta,name:meta.name||"Untitled",native_name:meta.native_name||"",codename:meta.codename||"",world:meta.world||"",image:meta.image||"",updated:meta.updated||"",updated_at:meta.updated_at||"",tags:Array.isArray(meta.tags)?meta.tags:[],sections,raw:text,_file:file};
}
...
function showHome(updateHash=true){if(updateHash)setHash("home");state.filterWorld=null;navActive("home");setCrumb("HOME");const h=homeSettings,recent=state.posts.map((p,i)=>({p,i})).sort((a,b)=>{const ak=a.p.updated_at||a.p.updated||"",bk=b.p.updated_at||b.p.updated||"";return bk.localeCompare(ak)}).slice(0,5);contentEl.innerHTML=`<section class="hero"><div class="eyebrow">${esc(h.eyebrow)}</div><h1>${esc(h.title)} <em>${esc(h.accent)}</em></h1><p>${esc(h.description)}</p><div class="stats"><span><b>${state.posts.length}</b> PERSONAS</span><span><b>${new Set(state.posts.map(p=>p.world).filter(Boolean)).size}</b> WORLDS</span></div></section>${isOwnerMode()?`<div class="home-edit-row"><button class="home-edit-btn" onclick="editHome()">✎ EDIT HOME</button></div>`:""}<div class="section-head"><h2>PERSONAS</h2><span>ARCHIVE INDEX</span></div><div class="card-grid">${state.posts.map(card).join("")}</div><div class="section-head"><h2>RECENTLY UPDATED</h2><span>LAST CHANGES</span></div><div class="recent-list">${recent.map(({p,i})=>`<div class="recent-row" onclick="showDetail(${i})"><span>◇ ${esc(p.name)} / ${esc(p.codename)}</span><span class="recent-date">${esc(p.updated||"—")}</span></div>`).join("")}</div>`;closeSide()}
function getFiltered(){const q=state.query.toLowerCase();return state.posts.map((p,i)=>({p,i})).filter(({p})=>{const hay=[p.name,p.native_name,p.codename,p.world,p.catchphrase,p.summary,...p.tags,...p.sections.map(s=>s.title+" "+s.content)].join(" ").toLowerCase();return(!q||hay.includes(q))&&(!state.filterWorld||p.world===state.filterWorld)})}
...
function showDetail(i,updateHash=true){const p=state.posts[i];if(!p)return;if(updateHash)setHash("persona/"+encodeURIComponent(postSlug(p)));navActive("");setCrumb(p.codename||p.name);const sections=p.sections.map((s,si)=>{const prompt=s.type==="copy"||/PROMPT|OOC/i.test(s.title);return`<section class="doc-section"><h2>${esc(s.title)}</h2>${prompt?`<div class="prompt-box"><div class="prompt-top"><span>COPY SECTION</span><button class="copy-btn" onclick="copyPrompt(${i},${si},this)">COPY</button></div><pre>${esc(s.content)}</pre></div>`:`<div class="prose">${esc(s.content)}</div>`}</section>`}).join("");contentEl.innerHTML=`<button class="back-btn" onclick="showPersonas()">← BACK TO INDEX</button>${isOwnerMode()?`<div class="detail-tools"><button class="delete-entry-btn" onclick="deletePersona(${i})">DELETE ENTRY</button><button class="edit-entry-btn" onclick="editor(${i})">✎ EDIT ENTRY</button></div>`:""}<div class="detail-head">${art(p,"portrait")}<div class="detail-title"><div class="eyebrow">${esc(p.world||"UNFILED")}</div><div class="detail-name-block"><div class="code">${esc(p.codename)}</div><div class="detail-name-row"><h1>${esc(p.name)}</h1>${p.native_name?`<div class="native-name">${esc(p.native_name)}</div>`:""}</div></div><div class="detail-tags">${p.tags.map(t=>`<span class="tag-chip">#${esc(t)}</span>`).join("")}</div>${(p.catchphrase||p.summary)?`<div class="character-intro">${p.catchphrase?`<div class="character-catchphrase">${esc(p.catchphrase)}</div>`:""}${p.summary?`<div class="character-summary">${esc(p.summary)}</div>`:""}</div>`:""}</div></div>${profileGrid(p)}<div class="detail-body">${sections}</div>`;window.scrollTo(0,0);closeSide()}
