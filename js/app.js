...
function renderSide(){const worlds=[...new Set(state.posts.map(p=>p.world).filter(Boolean))].sort();worldList.innerHTML=worlds.map(w=>`<button class="world-btn" onclick="filterWorld('${esc(w)}')">⌞ ${esc(w)}</button>`).join("")}
...
function showHome(){state.filterWorld=null;navActive("home");setCrumb("HOME");const h=getHomeSettings();const recent=state.posts.map((p,i)=>({p,i})).sort((a,b)=>(b.p.updated||"").localeCompare(a.p.updated||"")).slice(0,5);contentEl.innerHTML=`<section class="hero"><div class="eyebrow">${esc(h.eyebrow)}</div><h1>${esc(h.title)} <em>${esc(h.accent)}</em></h1><p>${esc(h.description)}</p><div class="stats"><span><b>${state.posts.length}</b> PERSONAS</span><span><b>${new Set(state.posts.map(p=>p.world).filter(Boolean)).size}</b> WORLDS</span></div></section><div class="home-edit-row"><button class="home-edit-btn" onclick="editHome()">✎ EDIT HOME</button></div><div class="section-head"><h2>PERSONAS</h2><span>ARCHIVE INDEX</span></div><div class="card-grid">${state.posts.map(card).join("")}</div><div class="section-head"><h2>RECENTLY UPDATED</h2><span>LAST CHANGES</span></div><div class="recent-list">${recent.map(({p,i})=>`<div class="recent-row" onclick="showDetail(${i})"><span>◇ ${esc(p.name)} / ${esc(p.codename)}</span><span class="recent-date">${esc(p.updated||"—")}</span></div>`).join("")}</div>`;closeSide()}
function editHome(){const h=getHomeSettings();navActive("");setCrumb("EDIT HOME");contentEl.innerHTML=`<div class="editor-wrap"><div class="eyebrow">✦ Archive Settings</div><h1 class="editor-title">Edit Home</h1><div class="form-grid"><div class="field full"><label>EYEBROW</label><input id="homeEyebrow" value="${esc(h.eyebrow)}"></div><div class="field"><label>TITLE</label><input id="homeTitle" value="${esc(h.title)}"></div><div class="field"><label>ACCENT · violet text</label><input id="homeAccent" value="${esc(h.accent)}"></div><div class="field full"><label>DESCRIPTION</label><textarea id="homeDescription">${esc(h.description)}</textarea></div></div><div class="editor-actions"><button class="action-btn primary" onclick="saveHomeEditor()">SAVE HOME</button><button class="action-btn" onclick="showHome()">CANCEL</button></div><div class="editor-note">현재 v0.2.2에서는 이 브라우저에 저장됩니다. v0.3에서 GitHub 저장 기능과 연결하면 사이트 설정 파일에도 영구 저장되도록 확장할 예정입니다.</div></div>`;closeSide()}
function saveHomeEditor(){saveHomeSettings({eyebrow:document.getElementById("homeEyebrow").value.trim(),title:document.getElementById("homeTitle").value.trim(),accent:document.getElementById("homeAccent").value.trim(),description:document.getElementById("homeDescription").value.trim()});showHome()}
function showPersonas(){navActive("personas");setCrumb("PERSONAS");const list=getFiltered();content.innerHTML=`<section class="hero"><div class="eyebrow">Archive Index</div><h1>Persona <em>Index</em></h1><p>${state.filterWorld?`WORLD · ${esc(state.filterWorld)}`:"전체 페르소나"}${state.query?` · SEARCH "${esc(state.query)}"`:""}</p></section><div class="section-head"><h2>${list.length} ENTRIES</h2><span>${state.filterWorld||"ALL"}</span></div><div class="card-grid">${list.map(({p,i})=>card(p,i)).join("")||'<div class="empty">조건에 맞는 페르소나가 없습니다.</div>'}</div>`;closeSide()}
...
function editor(prefill=null){navActive("");setCrumb(prefill?"EDIT ENTRY":"NEW ENTRY");const p=prefill||{tags:[],sections:[],updated:new Date().toISOString().slice(0,10)};content.innerHTML=`<div class="editor-wrap"><div class="eyebrow">✦ Archive Editor</div><h1 class="editor-title">${prefill?"Edit":"New"} Persona</h1><div class="form-grid">
...
<div class="editor-actions"><button class="action-btn" onclick="addSection()">＋ ADD SECTION</button><button class="action-btn primary" onclick="downloadMd()">DOWNLOAD .MD</button><button class="action-btn" onclick="importMd.click()">IMPORT .MD</button><button class="action-btn" onclick="previewEditor()">PREVIEW</button></div><div class="editor-note">v0.2에서는 PROFILE만 고정 항목이며, 아래 섹션은 CHARACTER / BACKGROUND / RELATIONSHIP / PROMPT 등 원하는 제목으로 자유롭게 추가할 수 있습니다.</div></div>`;(p.sections||[]).forEach(s=>addSection(s.title,s.content));if(!p.sections?.length){addSection("CHARACTER","");addSection("BACKGROUND","");addSection("AI CHAT PROMPT","")}closeSide()}
function addSection(title="",body=""){const id=customCount++;const wrap=document.createElement("div");wrap.className="custom-section";wrap.dataset.section=id;wrap.innerHTML=`<input class="section-title" placeholder="SECTION TITLE" value="${esc(title)}"><button class="remove-section" onclick="this.parentElement.remove()">✕</button><textarea class="section-body" placeholder="내용을 입력하세요.">${esc(body)}</textarea>`;sectionsHost.appendChild(wrap)}
...
document.querySelectorAll(".nav-item").forEach(b=>b.onclick=()=>b.dataset.view==="home"?showHome():showPersonas());searchInput.addEventListener("input",e=>{state.query=e.target.value.trim();showPersonas()});newEntryBtn.onclick=()=>editor();menuBtn.onclick=()=>sidebar.classList.toggle("open");themeBtn.onclick=()=>document.body.classList.toggle("light");importMd.onchange=async e=>{const f=e.target.files[0];if(f)editor(parseFrontMatter(await f.text()));e.target.value=""};loadPosts();