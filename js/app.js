...
function showDetail(i){const p=state.posts[i];if(!p)return;navActive("");setCrumb(p.codename||p.name);const sections=p.sections.map((s,si)=>{const prompt=/PROMPT|OOC/i.test(s.title);return`<section class="doc-section"><h2>${esc(s.title)}</h2>${prompt?`<div class="prompt-box"><div class="prompt-top"><span>ARCHIVED PROMPT</span><button class="copy-btn" onclick="copyPrompt(${i},${si},this)">COPY</button></div><pre>${esc(s.content)}</pre></div>`:`<div class="prose">${esc(s.content)}</div>`}</section>`}).join("");contentEl.innerHTML=`<button class="back-btn" onclick="showPersonas()">← BACK TO INDEX</button><div class="detail-tools"><button class="edit-entry-btn" onclick="editor(${i})">✎ EDIT ENTRY</button></div><div class="detail-head">${art(p,"portrait")}<div class="detail-title"><div class="eyebrow">${esc(p.world||"UNFILED")}</div><h1>${esc(p.name)}</h1>${p.native_name?`<div class="native-name">${esc(p.native_name)}</div>`:""}<div class="code">${esc(p.codename)}</div><div class="detail-tags">${p.tags.map(t=>`<span class="tag-chip">#${esc(t)}</span>`).join("")}</div></div></div>${profileGrid(p)}<div class="detail-body">${sections}</div>`;window.scrollTo(0,0);closeSide()}
...
<div class="editor-subhead">FREE SECTIONS</div><div id="sectionsHost" class="field full"></div></div><div class="editor-actions"><button class="action-btn" onclick="addSection()">＋ ADD SECTION</button><button class="action-btn primary" onclick="savePersonaToGitHub()">SAVE ENTRY</button><button class="action-btn" onclick="previewEditor()">PREVIEW</button></div><div class="save-status" id="saveStatus"></div></div>`;
(p.sections||[]).forEach(s=>addSection(s.title,s.content));if(!p.sections?.length){addSection("CHARACTER","");addSection("BACKGROUND","");addSection("AI CHAT PROMPT","")}
