...
async function ghDelete(path,message){const c=requireGitHub(),old=await ghGet(path);if(!old?.sha)return false;await ghRequest(path,{method:"DELETE",body:JSON.stringify({message,sha:old.sha,branch:c.branch||"main"})});return true}
async function deletePersona(i){const p=state.posts[i];if(!p)return;const filename=p._file;if(!filename){alert("이 게시글의 파일명을 확인할 수 없어 삭제하지 않았습니다.");return}const ok=confirm(`“${p.name}”을(를) 정말 삭제할까요?\n\n게시글은 Archive에서 제거되며, 전용 업로드 이미지가 다른 게시글에서 사용되지 않는 경우 이미지도 함께 삭제됩니다.`);if(!ok)return;try{requireGitHub();const btn=document.querySelector(".delete-entry-btn");if(btn){btn.disabled=true;btn.textContent="DELETING…"}
 const idx=await ghGet("posts/index.json");let list=[];if(idx?.content)list=JSON.parse(new TextDecoder().decode(Uint8Array.from(atob(idx.content.replace(/\n/g,"")),c=>c.charCodeAt(0))));list=list.filter(f=>f!==filename);await ghPut("posts/index.json",utf8b64(JSON.stringify(list,null,2)+"\n"),`Remove persona from index: ${p.name}`);
 ...
 state.posts.splice(i,1);renderSide();showHome();alert(`“${p.name}” 삭제 완료! GitHub Pages 재배포 후 공개 페이지에도 반영됩니다.`)
